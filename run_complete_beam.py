...
def process_all_projects(dataset_root: str = "/data/david/project/beam/dataset", 
    ...
    logger.info(f"Found {len(data_json_files)} projects:")
    for path in data_json_files:
        project_name = get_project_name_from_path(path)
        logger.info(f"  - {project_name}: {path}")
    
    logger.info("\n" + "=" * 80)
    
    # 统计信息
    total_projects = len(data_json_files)
    processed_projects = 0
    failed_projects = []
    
    # 遍历每个项目
    for idx, dataset_path in enumerate(data_json_files, 1):
        project_name = get_project_name_from_path(dataset_path)
