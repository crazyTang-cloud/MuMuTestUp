...
class EclipseJDTLS(LanguageServer):
    ...
    def setupRuntimeDependencies(
        ...
        # Try to set execute permission, but don't fail if not permitted
        try:
            os.chmod(jre_path, stat.S_IEXEC)
        except (PermissionError, OSError) as e:
            # If we can't set permissions, assume the file already has execute permission
            # This can happen on some file systems or in restricted environments
            import logging
            logger.log(f"Warning: Could not set execute permission on {jre_path}: {e}", logging.WARNING)
            logger.log("Assuming Java executable already has correct permissions...", logging.INFO)
