...
class LanguageServer:
    ...
    def get_text_between_positions(
        self,
        relative_file_path: str,
        start: multilspy_types.Position,
        end: multilspy_types.Position,
        ...
        """
        Get the text between the given start and end positions in the given file.
        """
        if not self.server_started:
            self.logger.log(
                "get_text_between_positions called before Language Server started",
                logging.ERROR,
            )
            raise MultilspyException("Language Server not started")
        ...
        file_buffer = self.open_file_buffers[uri]
        start_idx = TextUtils.get_index_from_line_col(
            file_buffer.contents, start["line"], start["character"]
        )
        end_idx = TextUtils.get_index_from_line_col(
            file_buffer.contents, end["line"], end["character"]
        )
        return file_buffer.contents[start_idx:end_idx]
...
class SyncLanguageServer:
    ...
    def get_text_between_positions(
        ...
        Get the text between the given start and end positions in the given file.
        """
        return self.language_server.get_text_between_positions(
            relative_file_path, start, end
        )
